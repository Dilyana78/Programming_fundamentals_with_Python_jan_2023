...
for ticket in tickets:
    ...
    for symbol in symbols:
        ...
        if overall_result > winning_result:
            winning_result = overall_result
            best_symbol = symbol

    is_winning = winning_result >= 6
    is_jackpot = is_winning and winning_result == 10
    if is_jackpot:
        print(f'ticket "{ticket}" - {winning_result}{best_symbol} Jackpot!')
    elif is_winning:
        print(f'ticket "{ticket}" - {winning_result}{best_symbol}')
    else:
